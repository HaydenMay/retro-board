...
test("readiness is private to its member except that admins can review the retro roster", () => {
  ...
  assert.match(member[".validate"], /hasChildren\(\['readyAt'\]\)/);
  assert.equal(member.readyAt[".validate"], "newData.isNumber()");
  assert.equal(member.$other[".validate"], false);
  assert.doesNotMatch(readinessJson, /numChildren\s*\(/);
});
